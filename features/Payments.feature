Feature: Payments
  ...
  Scenario: Validating Payments scenarios-Sign in at checkout, verify contact Information displayed, confirm order placement with Visa
    ...
    And user tap on Place Order button in DSG application
    And user order should be processed and should displayed the Order Confirmation Screen with payment Method in DSG application